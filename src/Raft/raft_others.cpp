...
Raft::Raft() :raftTimer(INITIAL_STATE)
{
...
#ifdef JPDEBUG
		std::cout << "waiting for every node is ready " << std::endl;
		std::cout << "the number of node that is online is " << nodeIdList.size() << std::endl;
#endif // JPDEBUG
	...
	resetLeaderPara();
	fileWriter.open("raft_"+std::to_string(nodeId));
#ifdef RAFT_SHOW
...
void Raft::receiveExaminer() 
{
	...
	//cout << "checking" << endl;
	if (nodeState != LEADER) return;
	if (stateRead_guard()==CHECK_TO_RAFT) {//如果check传来信息
		cout << "Recieve the message from Check" << endl;
		stateWrite_guard(RAFT_GET);
		logs.addAction(action, currentTerm);
		writeSaid("Recieve the message from Check");
		writeSaid("Message is " + action);
	}
}


void Raft::answerExaminer() 
{
	if (RAFT_GET) {
		actionWrite_guard("");
		std::cout << "answer the Check" << std::endl;
		stateWrite_guard(RAFT_TO_CHECK);
		writeSaid("answer the check");
	}
