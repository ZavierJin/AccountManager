...
bool Client::Load_user(std::string username) //载入当前用户数据
{
	...
	if (username != clnt.username) //当输入的用户名与当前用户不同时，重新载入当前用户数据，否则无需重新载入
	{
		...
		filename = "Users" + std::to_string(Global_nodeId) + "\\Users\\" + filename + ".txt";
...
{
	using namespace std;
	string filename = "Users" + std::to_string(Global_nodeId) + "\\users.txt";
	string temp;
...
{
	using namespace std;
	string filename = "Users" + std::to_string(Global_nodeId);
	if (0 != _access(filename.c_str(), 0))
	{
		//若文件夹不存在，则创建此文件夹
		_mkdir(filename.c_str());   // 返回 0 表示创建成功，-1 表示失败
	}
	filename = "Users" + std::to_string(Global_nodeId) + "\\Users";
	if (0 != _access(filename.c_str(), 0))
	{
		//若文件夹不存在，则创建此文件夹
		_mkdir(filename.c_str());   // 返回 0 表示创建成功，-1 表示失败
...
bool Client::Register(std::string s_userinfo) //用户注册，即新建用户文件
{
	...
	ofstream fout;
	string filename = username;
	filename = "Users" + std::to_string(Global_nodeId) + "\\Users\\" + filename + ".txt"; //打开用户的文件，写入信息
	fout.open(filename.c_str(), ios_base::out);
	if (!fout.is_open()) {
		return false; //若失败则终止
	}
	...
	filename = "Users" + std::to_string(Global_nodeId) + "\\users.txt";
