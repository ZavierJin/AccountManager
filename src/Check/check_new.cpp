...
void check_Register() {
	...
	if (!client.Check_id(userinfo.username))  //检验用户名是否存在
	{
		...
		if (checkState())//需要raft的返回值
		{
			...
			com.sendAnswer(reg_ans, req2->getAddress());
		}

		else
		{
			answer::RegisterAnswer reg_ans(0);
			com.sendAnswer(reg_ans, req2->getAddress());//注册失败
		}
	}
...
void check_SignIn() {
	...
	auto req2 = com.getRequest(request::Kind::SignIn);
	if (req2 == nullptr)	return;

	answer::FeedBack feb_ans(id == idRead_guard(), idRead_guard());
	com.sendAnswer(feb_ans, req2->getAddress());//返回是否是leader和Global_leaderId
	...
	if (client.Check_id(userid))  //检验用户名是否存在
	{
		if (client.Log_in(userid, userpassword)) //返回密码是否正确
		{
			...
			com.sendAnswer(log_ans, req2->getAddress());//登陆成功
		}
		else
		{
			answer::LogInAnswer log_ans(0);
			com.sendAnswer(log_ans, req2->getAddress());//密码错误
		}
	}
	else
	{
		answer::LogInAnswer log_ans(2);
		com.sendAnswer(log_ans, req2->getAddress());//用户名不存在
	}
...
void check_Deal()
{
	...
	auto req2 = com.getRequest(request::Kind::Deal);
	if (req2 == nullptr)	return;
	...
	if (client.Check_id(deal.payer) && client.Check_id(deal.payee))  //检验用户名是否存在
	{
		if (client.Check_fund(deal.payer, deal.change)) //检验余额是否充足
		{
			...
			if (checkState())
			{
				...
				com.sendAnswer(dea_ans, req2->getAddress());
			}//交易成功
			else
			{
...
void check_BalanceCheck()
{
	using namespace std;
	int  id = Global_nodeId;//分开读取？
	auto &com = computer::Computer::instance();

	auto req2 = com.getRequest(request::Kind::BalanceCheck);
	if (req2 == nullptr)	return;
	...
	if (client.Check_id(userid))  //检验用户名是否存在
	{
		...
		com.sendAnswer(bal_ans, req2->getAddress());
	}
	else
	{
...
void check_RecordCheck()
{
	using namespace std;
	int  id = Global_nodeId;//分开读取？
	auto &com = computer::Computer::instance();
	
	auto req2 = com.getRequest(request::Kind::RecordCheck);
	if (req2 == nullptr)	return;
	...
	{	
		...
		com.sendAnswer(rec_ans, req2->getAddress());
	}
	else
	{
